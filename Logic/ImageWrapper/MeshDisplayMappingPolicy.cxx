...
MeshDisplayMappingPolicy::GetNativeImageRangeForCurve()
{
  ...
  vtkIdType activeComp =
    prop->GetActiveVectorMode() == VectorMode::COMPONENT ? prop->GetActiveComponentId() : -1;
...
MeshDisplayMappingPolicy::SetColorMap(ColorMap *map)
{
  ...
  m_ColorMapObserverTag = Rebroadcaster::Rebroadcast(
    map, itk::ModifiedEvent(), m_Wrapper, WrapperDisplayMappingChangeEvent());
...
MeshDisplayMappingPolicy::SetIntensityCurve(IntensityCurveVTK *curve)
{
  ...
  m_IntensityCurveObserverTag = Rebroadcaster::Rebroadcast(
    curve, itk::ModifiedEvent(), m_Wrapper, WrapperDisplayMappingChangeEvent());

  m_Wrapper->InvokeEvent(WrapperDisplayMappingChangeEvent());
}

void
MeshDisplayMappingPolicy::UpdateActorMap(ActorPool *pool, unsigned int timepoint)
{
...
{
  auto prop = m_Wrapper->GetActiveDataArrayProperty();
  return prop ? prop->GetTDigest() : nullptr;
}
...
vtkLookupTable *
MeshDisplayMappingPolicy::GetLookupTable()
{
  UpdateLUT();
...
// ==================================================
GenericMeshDisplayMappingPolicy::GenericMeshDisplayMappingPolicy() {}

GenericMeshDisplayMappingPolicy::~GenericMeshDisplayMappingPolicy() {}

void
GenericMeshDisplayMappingPolicy::UpdateAppearance(ActorPool *pool, unsigned int)
{
  // Get active data array property
  auto prop = m_Wrapper->GetActiveDataArrayProperty();

  // This should never happen, check data property implementation
  assert(!prop || prop->GetType() != MeshDataType::COUNT);
  ...
  for (auto it = actorMap->begin(); it != actorMap->end(); ++it)
  {
    ...
    if (prop)
    {
      ...
      if (prop->GetType() == MeshDataType::POINT_DATA)
      {
        ...
        pointData->SetActiveAttribute(prop->GetName(), vtkDataSetAttributes::AttributeTypes::SCALARS);
      }
      else if (prop->GetType() == MeshDataType::CELL_DATA)
      {
        mapper->SetScalarModeToUseCellData();
        auto cellData = mapper->GetInput()->GetCellData();
        cellData->SetActiveAttribute(prop->GetName(), vtkDataSetAttributes::AttributeTypes::SCALARS);
      }
    ...
    else
    {
      ...
      actor->GetProperty()->SetColor(m_Wrapper->GetSolidColor().data_block());
    }
  }
}
...
void
GenericMeshDisplayMappingPolicy::UpdateLUT()
{
  ...
  for (auto i = 0u; i < numClr; ++i)
  {
    ...
    double rgbaD[4] = { rgbaC[0] * clrdiv, rgbaC[1] * clrdiv, rgbaC[2] * clrdiv, rgbaC[3] * clrdiv };
...
// ==================================================
LabelMeshDisplayMappingPolicy::LabelMeshDisplayMappingPolicy() {}

LabelMeshDisplayMappingPolicy::~LabelMeshDisplayMappingPolicy() {}

void
LabelMeshDisplayMappingPolicy ::SetColorLabelTable(ColorLabelTable *labelTable)
{
  m_ColorLabelTable = labelTable;

  Rebroadcaster::Rebroadcast(
    labelTable, SegmentationLabelPropertyChangeEvent(), m_Wrapper, WrapperDisplayMappingChangeEvent());
}

void
LabelMeshDisplayMappingPolicy::UpdateAppearance(ActorPool *pool, unsigned int)
{
  auto actorMap = pool->GetActorMap();
...
void
LabelMeshDisplayMappingPolicy::ConfigureLegend(vtkScalarBarActor *legend)
{
  legend->SetLookupTable(m_LookupTable);
...
void
LabelMeshDisplayMappingPolicy::UpdateLUT()
{
  ...
  for (auto cit = m_ColorLabelTable->begin(); cit != m_ColorLabelTable->end(); ++cit)
  {
