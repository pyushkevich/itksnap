...
::GenerateData()
{
  ...
  size_t iStart = (size_t)stride_image[0]*(size_t)xStartVoxel[0]
                ...
                + (size_t)stride_image[2]*(size_t)xStartVoxel[2];
...
IRISSlicer<RLEImage<TPixel, 3, CounterType>, TOutputImage>
::IRISSlicer()
{
    // Two inputs are allowed (second being the preview input)
    this->SetNumberOfIndexedInputs(2);
...
template< typename TPixel, typename CounterType, class TOutputImage>
typename IRISSlicer<RLEImage<TPixel, 3, CounterType>, TOutputImage>::InputImageType *
IRISSlicer<RLEImage<TPixel, 3, CounterType>, TOutputImage>
::GetPreviewInput()
{
    return const_cast<InputImageType *>(this->GetInput(1));
