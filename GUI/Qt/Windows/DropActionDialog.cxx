...
void DropActionDialog::SetDroppedFilename(QString name)
{
  ...
  if (isPolyData)
    ...
    if (GuidedMeshIO::can_read(fmt))
      {
      this->SetMode(FileDropMode::Mesh);
      }
    else
      {
      QMessageBox msgBox;
...
void DropActionDialog::SetMode(FileDropMode mode)
{
  switch(mode)
    {
    case FileDropMode::Image:
      ui->btnLoadMeshAsLayer->hide();
      ui->btnLoadMeshToTP->hide();

      ui->btnLoadMain->show();
      ui->btnLoadOverlay->show();
      ui->btnLoadSegmentation->show();
      ui->btnLoadAdditionalSegmentation->show();

      ui->btnLoadNew->show();
      break;
    case FileDropMode::Mesh:
      ui->btnLoadMeshAsLayer->show();

      ui->btnLoadMain->hide();
      ui->btnLoadOverlay->hide();
      ui->btnLoadSegmentation->hide();
      ui->btnLoadAdditionalSegmentation->hide();
