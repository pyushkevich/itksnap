...
public:
  ...
  void BuildBrush()
  {
    ...
    // Add to model
    std::vector<Vector2d> brush_rep;
    for(int i = 0; i < m_Walk->GetNumberOfPoints(); i++)
    {
      Vector2d x;
      m_Walk->GetPoint(i, x.data_block());
      brush_rep.push_back(x);
    }
    m_PaintbrushModel->SetBrushPoints(brush_rep);
  }
...
void
PaintbrushNewRenderer::RenderOverTiledLayer(AbstractNewRenderContext *context,
...
void
PaintbrushNewRenderer::InsertWithCollinearCheck(const Vector2d &head)
{
  Vector2d      a, b;
...
void
PaintbrushNewRenderer::BuildBrush(AbstractNewRenderContext *context)
{
  // Get the current properties
