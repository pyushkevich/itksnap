...
{
public:
  vtkTypeMacro(GlobalDecorationContextItem,
               GenericSliceContextItem) static GlobalDecorationContextItem *New();
  ...
  void DrawNicknames(vtkContext2D *painter)
  {
    ...
    auto  font_info =
      rps->MakeFont(elt->GetFontSize() * GetVPPR(), AbstractRendererPlatformSupport::SANS);
  ...
  void DrawOrientationLabels(vtkContext2D *painter)
  {
    ...
    // Create the font info
    auto *rps = AbstractRenderer::GetPlatformSupport();
    auto  font_info = rps->MakeFont(
      (int)(elt->GetFontSize() * vppr), AbstractRendererPlatformSupport::TYPEWRITER, true);
...
void
SliceWindowDecorationRenderer::AddContextItemsToGlobalOverlayScene(vtkContextScene *scene)
{
  if (m_Model)
  {
...
void
SliceWindowDecorationNewRenderer::RenderOverMainViewport(AbstractNewRenderContext *context)
{
  // Push an identity transform in logical pixel units
...
void
SliceWindowDecorationNewRenderer::RenderOverTiledLayer(AbstractNewRenderContext *context,
                                                       ImageWrapperBase         *base_layer,
                                                       const SubViewport        &vp)
{}

void
SliceWindowDecorationNewRenderer::DrawRuler(AbstractNewRenderContext *context)
{
  auto *as = m_Model->GetParentUI()->GetAppearanceSettings();
  auto *eltRuler = as->GetUIElement(SNAPAppearanceSettings::RULER);
  if (eltRuler->GetVisible())
...
void
SliceWindowDecorationNewRenderer::DrawNicknames(AbstractNewRenderContext *context)
{
  // Draw the nicknames
  DisplayLayoutModel *dlm = m_Model->GetParentUI()->GetDisplayLayoutModel();
  Vector2ui           layout = dlm->GetSliceViewLayerTilingModel()->GetValue();
...
std::list<std::string>
SliceWindowDecorationNewRenderer::GetDisplayText(ImageWrapperBase *layer)
{
  // Lines to be returned
  std::list<std::string> lines;
  int                    nc = layer->GetNumberOfComponents();
...
std::string
SliceWindowDecorationNewRenderer::CapStringLength(const std::string &str, size_t max_size)
{
  if (str.size() <= max_size)
    return str;
...
void
SliceWindowDecorationNewRenderer::DrawOrientationLabels(AbstractNewRenderContext *context)
{
