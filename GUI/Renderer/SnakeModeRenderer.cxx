...
{
public:
  vtkTypeMacro(SnakeModeContextItem, GenericSliceContextItem) static SnakeModeContextItem *New();
...
SnakeModeRenderer::SnakeModeRenderer() { m_Model = NULL; }

void
SnakeModeRenderer::AddContextItemsToTiledOverlay(vtkAbstractContextItem *parent, ImageWrapperBase *)
{
  if (m_Model)
  {
    vtkNew<SnakeModeContextItem> ci;
    ci->SetModel(m_Model);
    ci->SetSnakeWizardModel(m_Model->GetParentUI()->GetSnakeWizardModel());
    parent->AddItem(ci);
  }
}

void
SnakeModeNewRenderer::RenderOverTiledLayer(AbstractNewRenderContext *context,
...
void
SnakeModeNewRenderer::DrawBubbles(AbstractNewRenderContext *context)
{
  IRISApplication *app = m_Model->GetDriver();
  GlobalState     *gs = app->GetGlobalState();
  auto            *snake_wiz_model = m_Model->GetParentUI()->GetSnakeWizardModel();
