...
//    Value can be NULL
//
static char *SNAP_strnew(const char *val) {
...
//    Value can be NULL
//
static char *SNAP_strfree(char *val) {
  if ( val ) delete [] val;
  return(NULL);
}
...
//
#if !defined(WIN32)
static char *SNAP_strapp(char *s, const char *val) {
  if ( ! val ) {
    return(s);			// Nothing to append? return s
  }
  if ( ! s ) {
    return(SNAP_strnew(val));	// New string? return copy of val
  }
...
//
static void SNAP_chrcat(char *s, char c) {
  char tmp[2] = { c, '\0' };
  strcat(s, tmp);
}
