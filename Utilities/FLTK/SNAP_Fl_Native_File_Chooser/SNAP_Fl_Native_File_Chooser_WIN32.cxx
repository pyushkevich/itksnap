...
static void dnullcat(char*&wp, const char *string, int n = -1 ) {
  ...
  int inlen = (int)(( n < 0 ) ? strlen(string) : n);
  if ( ! wp ) {
    wp = new char[inlen + 4];
    *(wp+0) = '\0';
    *(wp+1) = '\0';
...
// CTOR
SNAP_Fl_Native_File_Chooser::SNAP_Fl_Native_File_Chooser(int val) {
  _btype           = val;
  _options         = NO_OPTIONS;
...
// SET TYPE OF BROWSER
void SNAP_Fl_Native_File_Chooser::type(int val) {
  _btype = val;
}

// GET TYPE OF BROWSER
int SNAP_Fl_Native_File_Chooser::type() const {
  return( _btype );
}

// SET OPTIONS
void SNAP_Fl_Native_File_Chooser::options(int val) {
  _options = val;
}

// GET OPTIONS
int SNAP_Fl_Native_File_Chooser::options() const {
  return(_options);
}
...
void SNAP_Fl_Native_File_Chooser::add_pathname(const char *s) {
  if ( ! _pathnames ) {
...
void SNAP_Fl_Native_File_Chooser::FreePIDL(ITEMIDLIST *pidl) {
...
void SNAP_Fl_Native_File_Chooser::ClearOFN() {
...
void SNAP_Fl_Native_File_Chooser::ClearBINF() {
  if ( _binf.pidlRoot ) {
...
void SNAP_Fl_Native_File_Chooser::Win2Unix(char *s) {
  for ( ; *s; s++ )
    if ( *s == '\\' ) *s = '/';
}

// CONVERT UNIX FRONTSLASHES TO WINDOWS BACKSLASHES
void SNAP_Fl_Native_File_Chooser::Unix2Win(char *s) {
  for ( ; *s; s++ )
    if ( *s == '/' ) *s = '\\';
}

// SHOW FILE BROWSER
int SNAP_Fl_Native_File_Chooser::showfile() {
  ClearOFN();
  clear_pathnames();
  size_t fsize = SNAP_MAX_PATH;
  ...
  if (_parsedfilt != NULL) {	// to convert a null-containing char string into a widechar string
    static WCHAR wpattern[SNAP_MAX_PATH];
    const char *p = _parsedfilt;
    while(*(p + strlen(p) + 1) != 0) p += strlen(p) + 1;
    p += strlen(p) + 2;
    MultiByteToWideChar(CP_UTF8, 0, _parsedfilt, (int)(p - _parsedfilt), wpattern, SNAP_MAX_PATH);
...
//  Unix2Win(_ofn.lpstrFile);
  ...
  if ( _directory ) {
    ...
    //
    _ofn.lpstrInitialDir    = new WCHAR[SNAP_MAX_PATH];
  ...
  //
  char oldcwd[SNAP_MAX_PATH];
  GetCurrentDirectory(SNAP_MAX_PATH, oldcwd);
  oldcwd[SNAP_MAX_PATH-1] = '\0';
...
//
int CALLBACK SNAP_Fl_Native_File_Chooser::Dir_CB(HWND win, UINT msg, LPARAM param, LPARAM data) {
  switch (msg) {
    case BFFM_INITIALIZED:
      if (data) ::SendMessage(win, BFFM_SETSELECTION, TRUE, data);
      break;
    case BFFM_SELCHANGED:
      TCHAR path[SNAP_MAX_PATH];
...
int SNAP_Fl_Native_File_Chooser::showdir() {
  ...
  // BUFFER
  char displayname[SNAP_MAX_PATH];
  _binf.pszDisplayName = displayname;
  // PRESET DIR
  char presetname[SNAP_MAX_PATH];
  if ( _directory ) {
...
//
int SNAP_Fl_Native_File_Chooser::show() {
...
const char *SNAP_Fl_Native_File_Chooser::errmsg() const {
  return(_errmsg ? _errmsg : "No error");
}

// GET FILENAME
const char* SNAP_Fl_Native_File_Chooser::filename() const {
  if ( _pathnames && _tpathnames > 0 ) return(_pathnames[0]);
  return("");
}

// GET FILENAME FROM LIST OF FILENAMES
const char* SNAP_Fl_Native_File_Chooser::filename(int i) const {
  if ( _pathnames && i < _tpathnames ) return(_pathnames[i]);
  return("");
}

// GET TOTAL FILENAMES CHOSEN
int SNAP_Fl_Native_File_Chooser::count() const {
  return(_tpathnames);
}
...
//
const char *SNAP_Fl_Native_File_Chooser::directory() const {
  return(_directory);
}
...
//    Can return NULL if none set.
//
const char *SNAP_Fl_Native_File_Chooser::title() const {
  return(_title);
}
...
//    Can return NULL if none set.
//
const char *SNAP_Fl_Native_File_Chooser::filter() const {
  return(_filter);
}
...
// ADD A FILTER
void SNAP_Fl_Native_File_Chooser::add_filter(const char *name_in,	// name of filter (optional: can be null)
...
//              Name     Wildcard
//
void SNAP_Fl_Native_File_Chooser::parse_filter(const char *in) {
  clear_filters();
  if ( ! in ) return;
...
void SNAP_Fl_Native_File_Chooser::filter_value(int i) {
  _ofn.nFilterIndex = i + 1;
}

// RETURN VALUE OF 'CURRENTLY SELECTED FILTER'
int SNAP_Fl_Native_File_Chooser::filter_value() const {
...
const char* SNAP_Fl_Native_File_Chooser::preset_file() const {
  return(_preset_file);
}

int SNAP_Fl_Native_File_Chooser::filters() const {
  return(_nfilters);
}
